    ...
    <div className="space-y-8">
      ...
      {results.map((result, index) => (
        <div key={index} className="space-y-4">
          <h3 className="text-lg font-medium">Scene: {result.sceneDescription}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {result.images.map((image) => (
              <Card key={image.id} className="group relative overflow-hidden glow-card">
                <div className="aspect-[9/5.5] relative">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <Button variant="secondary" size="sm" onClick={() => handleDownload(image.url, image.filename)}>
